...
`Filter` headers in the page meta block in order to supply the `filtered_pages` array for pages of your choice. This creates the
...
The `Filter` header also accepts a comma-separated list of tags, but instead specifies which pages end up in the `filtered_pages`
array. A page that specifies the header
`Filter: foo, bar` will receive in its `filtered_pages` array only pages having at least one of those two tags.
